...
class ARMST_BOT_PDA_COMPONENTClass : ScriptComponentClass
{
};
class ARMST_BOT_PDA_COMPONENT: ScriptComponent
{
	...
	void ArmstSendPdaStatus(IEntity owner)
	{
		SCR_CharacterIdentityComponent charIdentity = SCR_CharacterIdentityComponent.Cast(owner.FindComponent(SCR_CharacterIdentityComponent));
		if (!charIdentity)
    		return;
		
		
		GetName(m_sName, m_aNameParams);
		if (m_aNameParams && m_aNameParams.Count() > 0)
		{
		    string firstName = m_aNameParams[0]; // получаем первый элемент массива
		    // Используйте firstName в нужной строке
		}
		...
		if (chatComponent)
			...
			if (m_aNameParams && m_aNameParams.Count() > 0)
				{
				    string specificParam = m_aNameParams[0]; // или любое другое значение по индексу
				    string message = string.Format("Имя: %1, Остальные параметры: %2", specificParam, m_sName);
	...
	{		
			SCR_CharacterIdentityComponent scrCharIdentity = SCR_CharacterIdentityComponent.Cast(GetOwner().FindComponent(SCR_CharacterIdentityComponent));
			if (scrCharIdentity)
			{
			...
			{
				CharacterIdentityComponent charIdentity = CharacterIdentityComponent.Cast(GetOwner().FindComponent(CharacterIdentityComponent));
