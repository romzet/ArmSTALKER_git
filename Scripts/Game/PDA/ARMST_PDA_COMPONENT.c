    ...
    void UpdatePdaUI()
    {
       ...
       //  EditBoxToMessage.SetText("");
        ...
        if (m_StatsComponent)
		{
		    ...
		    {
		        ARMST_PLAYER_STATS_COMPONENT playerStats = ARMST_PLAYER_STATS_COMPONENT.Cast(m_User.FindComponent(ARMST_PLAYER_STATS_COMPONENT));
		        if (!playerStats)
		        {
		            Print("ARMST_PDA: Компонент ARMST_PLAYER_STATS_COMPONENT не найден на игроке.", LogLevel.ERROR);
		            Text_Fraction_Name.SetText("#Armst_unknown_faction");
		            return;
		        }
		        
		        ARMST_FACTION_LABEL factionKey = playerStats.GetFactionKey();
		        string factionDisplayName = "";
		        
		        // Определяем отображаемое название фракции на основе значения ARMST_FACTION_LABEL
		        switch (factionKey)
		        {
		            case ARMST_FACTION_LABEL.FACTION_STALKER:
		                factionDisplayName = "#Armst_neutral_character";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_BANDIT:
		                factionDisplayName = "#Armst_bandit_character";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_ARMY:
		                factionDisplayName = "#Armst_army_light";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_RENEGADE:
		                factionDisplayName = "#Armst_renegades_character";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_SCIENCES:
		                factionDisplayName = "#Armst_science_character";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_MERCENARIES:
		                factionDisplayName = "#Armst_faction_mercenaries";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_KB:
		                factionDisplayName = "#Armst_smuggler_character";
		                break;
		            case ARMST_FACTION_LABEL.FACTION_STORM:
		                factionDisplayName = "#Armst_storm_character";
		                break;
		            default:
		                factionDisplayName = "#Armst_unknown_faction";
		                Print(string.Format("ARMST_PDA: Неизвестная фракция для игрока: %1", factionKey), LogLevel.WARNING);
		                break;
		        }
