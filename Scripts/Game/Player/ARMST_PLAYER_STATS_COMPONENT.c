...
{
	[Attribute("1", UIWidgets.ComboBox, "Фракция", "", ParamEnumArray.FromEnum(ARMST_FACTION_LABEL), category: "Faction"),RplProp()]
	...
	[RplRpc(RplChannel.Reliable, RplRcver.Owner)]
	void Rpc_ArmstPlayerSetFactionDirect(ARMST_FACTION_LABEL factionKey)
	{
	...
	void SetFactionKey(ARMST_FACTION_LABEL factionKey)
	{
	    ...
	    switch (factionKey)
	    {
	        ...
	        case ARMST_FACTION_LABEL.FACTION_KB:
	            factionKeyStr = "FACTION_KB";
	            break;
	        case ARMST_FACTION_LABEL.FACTION_STORM:
	            factionKeyStr = "FACTION_STORM";
	            break;
	        default:
	            factionKeyStr = "FACTION_STALKER"; // Значение по умолчанию
	            break;
	    }
	    FactionKey faction = factionKeyStr;
	    // Устанавливаем фракцию через FactionAffiliationComponent
	    factionComponent.SetAffiliatedFactionByKey(faction);
	    Print(string.Format("Фракция игрока изменена на: %1", faction));
		Replication.BumpMe();
	}
    ...
    float m_fMaxRoofCheckDistance = 8;

    float m_fMaxWallCheckDistance = 8;
