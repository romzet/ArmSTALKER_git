...
class ARMST_DeathHandlerComponent : ScriptComponent
{
    ...
    [Attribute("10", UIWidgets.Slider, "Штраф за убийство союзника", "0 100 1", category: "Reputations")];
    float REPUTATION_LOSS_FRIENDLY_KILL;
    [Attribute("2", UIWidgets.Slider, "Бонус за убийство врага", "0 100 1", category: "Reputations")];
    ...
    void ProcessDeath()
    {
        ...
        Print("ARMST_DeathHandlerComponent started");
        ...
        if (characterFaction == "FACTION_STALKER" || characterFaction == "FACTION_BANDIT" || characterFaction == "FACTION_MERCENARIES" || characterFaction == "FACTION_SCIENCES" || characterFaction == "FACTION_ARMY" || characterFaction == "FACTION_STORM" || characterFaction == "FACTION_KB")
        {
            ...
            if (m_LastInstigatorEntity)
            {
       		 Print(m_LastInstigatorEntity);
                ARMST_PLAYER_STATS_COMPONENT playerStats = ARMST_PLAYER_STATS_COMPONENT.Cast(m_LastInstigatorEntity.FindComponent(ARMST_PLAYER_STATS_COMPONENT));
                ARMST_PLAYER_REPUTATIONS_COMPONENT reputations = ARMST_PLAYER_REPUTATIONS_COMPONENT.Cast(m_LastInstigatorEntity.FindComponent(ARMST_PLAYER_REPUTATIONS_COMPONENT));
                ...
                if (playerStats)
                {
                    ...
                    if (EntityUtils.IsPlayer(m_LastInstigatorEntity))
                    {
						Print("m_LastInstigatorEntity is player");
                        // Проверяем дружественность фракций
                        SCR_ChimeraCharacter ownerChar = SCR_ChimeraCharacter.Cast(owner);
                        SCR_ChimeraCharacter playerChar = SCR_ChimeraCharacter.Cast(m_LastInstigatorEntity);
                        
                        if (ownerChar && playerChar)
                        {
                            if (ownerChar.GetFaction().IsFactionFriendly(playerChar.GetFaction()))
                            {
                                // Если игрок убил союзника (дружественная фракция), уменьшаем репутацию у соответствующей фракции
                                if (reputations)
                                {
                                    ARMST_FACTION_LABEL factionKey = GetFactionKeyFromString(characterFaction);
                                    if (factionKey)
                                    {
                                        reputations.Rpc_ChangeReputation(factionKey, -REPUTATION_LOSS_FRIENDLY_KILL);
                                        Print("ARMST_DeathHandlerComponent: Decreased reputation for faction " + characterFaction + " by " + REPUTATION_LOSS_FRIENDLY_KILL + " due to friendly kill.", LogLevel.NORMAL);
                                    }
                                }
                            }
                            else
                            {
                                // Если игрок убил врага, увеличиваем репутацию у фракции игрока
                                if (reputations)
                                {
                                    ARMST_FACTION_LABEL playerFactionKey = GetFactionKeyFromString(m_LastInstigatorFaction);
                                    ARMST_FACTION_LABEL factionKey = GetFactionKeyFromString(characterFaction);
                                    if (playerFactionKey)
                                    {
                                        reputations.Rpc_ChangeReputation(playerFactionKey, REPUTATION_GAIN_ENEMY_KILL);
                                        reputations.Rpc_ChangeReputation(factionKey, -REPUTATION_LOSS_FRIENDLY_KILL);
                                        Print("ARMST_DeathHandlerComponent: Increased reputation for player's faction " + m_LastInstigatorFaction + " by " + REPUTATION_GAIN_ENEMY_KILL + " due to enemy kill.", LogLevel.NORMAL);
                                    }
                                }
    ...
    ARMST_FACTION_LABEL GetFactionKeyFromString(string factionKey)
    {
        ...
        Print("ARMST_DeathHandlerComponent: Unknown faction key: " + factionKey, LogLevel.WARNING);
